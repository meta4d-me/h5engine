...
namespace m4m.io
{
    ...
    export class SerializeDependent
    {
        ...
        static GetAssetContent(asset: any)
        {
            ...
            if (asset instanceof m4m.framework.material){
                ...
                if(t.lastIndexOf('.mat.json') == -1) {
                    return { "name": names[0] + ".mat.json", "value": (asset as m4m.framework.material).save(), "type": SaveAssetType.NameAndContent };
                }
                return { "name": t, "value": (asset as m4m.framework.material).save(), "type": SaveAssetType.NameAndContent };
            }
        ...
        static GetAssetUrl(asset: any, assetMgr: any)
        {
            ...
            if (asset instanceof m4m.framework.material)
            {
                ...
                let ass = (asset as m4m.framework.material);
                ...
                for (let newKey in ass.statedMapUniforms)
                {
                    ...
                    {
                        if ((_texture as m4m.framework.texture).defaultAsset == true)
                            continue;
    ...
    export function deSerializeOtherType(serializedObj: any, instanceObj: any, key: string, assetMgr: any, bundlename: string = null)
    {
        ...
        else
        {
            ...
            {
                let _newInstance;
                let componentType = m4m_reflect_root["__gdmeta__"][type];
                if (!componentType)
                {
                    console.warn(instanceObj);
                    console.warn(type);
                    return console.warn(`无法找到组件:${m4m_reflect_root["__gdmeta__"][type]}`);
                }
