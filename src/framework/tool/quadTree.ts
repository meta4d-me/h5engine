namespace m4m.framework {
    ...
    class qtNode {
        ...
        private split() {
            ...
            let r_0 = m4m.math.pool.new_rect(cx, y, sWidth, sHeight);
            let r_1 = m4m.math.pool.new_rect(x, y, sWidth, sHeight);
            let r_2 = m4m.math.pool.new_rect(x, cy, sWidth, sHeight);
            let r_3 = m4m.math.pool.new_rect(cx, cy, sWidth, sHeight);
            this.nodes.push(
        ...
        */
        ...
        retrieve(rect: math.rect, outRects: math.rect[]) {
            ...
            if (this.nodes.length) {
                ...
                if (index !== -1) {
                    ...
                    arr.forEach(element => {
                        if(element) m4m.math.pool.delete_rect(element);
                    });
                }
            }
        }
        ...
        private rectCarve(src: math.rect, cx: number, cy: number): math.rect[] {
            ...
            // 只切割X方向
                ...
                result.push(
                    m4m.math.pool.new_rect(src.x, src.y, dX, src.h),
                    m4m.math.pool.new_rect(cx, src.y, src.w - dX, src.h)
                );
            
            // 只切割Y方向
            } else if (carveY) {
                result.push(
                    m4m.math.pool.new_rect(src.x, src.y, src.w, dY),
                    m4m.math.pool.new_rect(src.x, cy, src.w, src.h - dY)
                );
            }
