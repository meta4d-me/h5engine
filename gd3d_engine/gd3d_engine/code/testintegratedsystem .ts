class test_integratedsystem implements IState
{
    ...
    {
        let name = "city";//10102_Training_s
        this.app.getAssetMgr().load("res/shader.assetbundle.json", gd3d.framework.AssetTypeEnum.Auto, (_state) =>
        {
            if (_state.isfinish)
            {
    ...
    private loadEffect(laststate: gd3d.framework.taskstate, state: gd3d.framework.taskstate)
    {
        ...
        this.effectListBoxs.onSelectItem = (name: string) =>
        {
            if (this.currentEffectName != name)
            {
                ...
                this._loadEffect("res/particleEffect/" + name + "/" + name + ".assetbundle.json", name + ".effect.json");
            }
        };
    ...
    private addEffectToPlayList(effectName: string)
    {
        ...
        btn.onclick = () =>
        {
            ...
            this.role.removeChild(this.currentEffectNameList[effectName + ".effect.json"]);
    ...
    private _loadEffect(fullName: string, name: string)
    {
        ...
        this.app.getAssetMgr().load(fullName, gd3d.framework.AssetTypeEnum.Auto, (_state) =>
        {
            if (_state.isfinish)
            {
                ...
                this.effect.gameObject.transform.localEulerAngles = new gd3d.math.vector3(0,45,0);
                this.role.gameObject.transform.localTranslate = new gd3d.math.vector3(29.48158, 27.01, 25.16527);
    ...
    private setFunctionArea()
    {
        ...
        btn_play.onclick = () =>
        {
            ...
            // if (this.effect.gameObject.transform != null)
            // {
            //     this.effect.gameObject.transform.dispose();
            // }
            this._loadEffect("res/particleEffect/" + this.currentEffectName + "/" + this.currentEffectName + ".assetbundle.json", this.currentEffectName + ".effect.json");
        }
        ...
        btn_clear.onclick = () =>
        {
            ...
            if (this.role)
            {
                ...
                {
                    console.log("this.roleLength: " + this.roleLength);
                    // let effectLength = this.role.children.length - 1-this.roleLength;
                    for (let i = this.role.children.length - 1; i > this.roleLength - 1; i--)
                    {
                        this.role.children[i].dispose();
                    }
                    // this.role.children.length = 0;
                }
            }
    ...
    update(delta: number)
    {
        ...
        if (this.role != null) 
        {
            let objCam = this.role.gameObject.transform;
