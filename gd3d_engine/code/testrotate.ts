namespace t
{
    ...
    export class TestRotate implements IState
    {
        ...
        {
            this.app.getAssetMgr().load("res/resources/1.pvr", gd3d.framework.AssetTypeEnum.Auto, (s) =>
            {
                if (s.isfinish)
                {
                    state.finish = true;
            ...
            //添加一个盒子
            {
                //添加一个盒子
                {
                    ...
                    if (sh != null)
                    {
                        ...
                        let texture = this.app.getAssetMgr().getAssetByName("1.pvr") as gd3d.framework.texture;
                        if (texture == null)
