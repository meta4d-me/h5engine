...
class test_pick_boxcollider implements IState
{
    ...
    start(app: gd3d.framework.application)
    {
        ...
        let name = names[1];
        demoTool.loadbySync(`newRes/shader/MainShader.assetbundle.json`, this.astMgr).then(() =>
        {
            this.loadScene(name);
        });
        ...
        //light
        let lObj = new gd3d.framework.transform();
        gd3d.math.quatFromEulerAngles(0,45,60,lObj.localRotate);
        let l = lObj.gameObject.addComponent("light") as gd3d.framework.light;
    ...
    private loadScene(assetName: string, isCompress = false)
    {
        ...
        let addScene = () =>
        {
            ...
            if (beAddScene)
            {
                var _scene: gd3d.framework.rawscene = this.app.getAssetMgr().getAssetByName(assetName + ".scene.json", `${assetName}.assetbundle.json`) as gd3d.framework.rawscene;
        ...
        this.app.getAssetMgr().load(`newRes/pfb/scene/${assetName}/${assetName}.assetbundle.json`, gd3d.framework.AssetTypeEnum.Auto, (s1) =>
        {
            if (s1.isfinish)
            {
                addScene();
    ...
    private rayCollider(): gd3d.framework.pickinfo
    {
        ...
        let mask = gd3d.framework.cullingmaskutil.layerToMask(this.pickLayer);
