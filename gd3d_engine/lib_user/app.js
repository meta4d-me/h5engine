                    ...
                    if (s.isfinish) {
                        ...
                        for (var i = -count; i <= count; i++) {
                            for (var j = -count; j <= count; j++) {
                                ...
                                var ap_1 = trans.gameObject.getComponent("aniplayer");
                                _this.aniplayers.push(ap_1);
                            }
                        }
                        var ap_2 = _this.aniplayers[0];
                        var list = ap_2.awaitLoadClipNames();
                        var resPath = _this.resPath;
                        var cname_1 = "";
                        if (list.length > 0) {
                            cname_1 = list[1];
                            ap_2.addClipByNameLoad(_this.app.getAssetMgr(), resPath, cname_1, function (sta, clipName) {
                                if (sta.isfinish) {
                                    var clip_1 = ap_2.getClip(cname_1);
        ...
        DragonTest.prototype.loadLongPrefab = function (laststate, state) {
            ...
            this.app.getAssetMgr().load("res/prefabs/" + resName + "/" + resName + ".assetbundle.json", gd3d.framework.AssetTypeEnum.Auto, function (s) {
                if (s.isfinish) {
                    ...
                    var ap_3 = _this.dragon.gameObject.getComponent("aniplayer");
...
var test_ParticleSystem = (function () {
    ...
    test_ParticleSystem.prototype.start = function (app) {
        ...
        hoverc.scaleSpeed = 0.1;
        hoverc.lookAtPoint = new gd3d.math.vector3(0, 2.5, 0);
        var mat_white = new gd3d.framework.material("white");
