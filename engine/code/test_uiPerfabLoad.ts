...
class test_uiPerfabLoad implements IState
{
    ...
    private doLoad(name)
    {
        ...
        let prefabName = name;
        // 加载 ui  预制体
        this.assetMgr.load(`res/prefabs/UI/${prefabName}/${prefabName}.assetbundle.json`, m4m.framework.AssetTypeEnum.Auto, (s1) =>
        {
            if (s1.isfinish)
            {
    ...
    {
        this.assetMgr.load("newRes/shader/shader.assetbundle.json", m4m.framework.AssetTypeEnum.Auto, (s) =>
        {
            if (s.isfinish)
            {
                state.finish = true;
