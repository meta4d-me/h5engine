...
class test_UI_Component implements IState
{
    ...
    private createUI(astState: m4m.framework.taskstate, state: m4m.framework.taskstate)
    {
        ...
        let _font = this.assetMgr.getAssetByName(fontjson) as m4m.framework.font;
        let lableW = 500;
        let lableH = 40;
        let lableStartX = 80;
        let lableStartY = 280;
        ...
        this.assetMgr.load("res/comp/comp.json.png", m4m.framework.AssetTypeEnum.Auto, (s) =>
        {
            if (s.isfinish)
            {
                this.assetMgr.load("res/comp/comp.atlas.json", m4m.framework.AssetTypeEnum.Auto, (s) =>
                {
                    ...
                    {
                        //加载字体资源
                        this.assetMgr.load("res/fonts/" + fontpng, m4m.framework.AssetTypeEnum.Auto, (s) =>
                        {
                            if (s.isfinish)
                            {
                                this.assetMgr.load("res/fonts/" + fontjson, m4m.framework.AssetTypeEnum.Auto, (s) =>
                                {
                                    this.assetMgr.load("res/zg03_256.png", m4m.framework.AssetTypeEnum.Auto, (s) =>
                                    {
                                        if (s.isfinish)
                                        {
                                            state.finish = true;
