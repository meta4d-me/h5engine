class test_load implements IState
{
    ...
    start(app: m4m.framework.application)
    {
        ...
        assetMgr.load("newRes/shader/Mainshader.assetbundle.json", m4m.framework.AssetTypeEnum.Auto, (state) =>
        {
            if (state.isfinish)
            {
                let name = "0001_shengyi_male";
                assetMgr.load(`newRes/pfb/model/${name}/${name}.assetbundle.json`, m4m.framework.AssetTypeEnum.Auto, (s) =>
                {
                    if (s.isfinish)
                    {
