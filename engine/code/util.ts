namespace util
{
    export function loadShader(assetMgr: m4m.framework.assetMgr)
    {
        ...
        {
            assetMgr.load("newRes/shader/Mainshader.assetbundle.json", m4m.framework.AssetTypeEnum.Auto, (_state) =>
            {
                if (_state.isfinish)
                {
                    resolve();
    ...
    export function loadModel(assetMgr: m4m.framework.assetMgr, modelName: string)
    {
        ...
        {
            assetMgr.load(`newRes/pfb/model/${modelName}/${modelName}.assetbundle.json`, m4m.framework.AssetTypeEnum.Auto, (s) =>
            {
                if (s.isfinish)
                {
