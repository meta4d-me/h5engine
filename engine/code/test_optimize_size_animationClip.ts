...
class test_optimize_size_animationClip implements IState
{
    ...
    start(app: m4m.framework.application)
    {
        ...
        this.app.getAssetMgr().load("./newRes/shader/MainShader.assetbundle.json", m4m.framework.AssetTypeEnum.Auto, (state) =>
        {
            if (state.isfinish)
            {
                this.app.getAssetMgr().load(`./newRes/pfb/elong_prefab/elong_prefab.assetbundle.json`, m4m.framework.AssetTypeEnum.Auto, (s) =>
                {
                    if (s.isfinish)
                    {
                        ...
                        this.app.getAssetMgr().load(`./newRes/pfb/elong_prefab/resources/Ready.FBAni.min.aniclip.bin`, m4m.framework.AssetTypeEnum.Aniclip, (s) =>
                        {
                            if (s.isfinish)
                            {
